...
function find_and_calculate_z_values {
    ...
    IFS=$'\n' Se_z_sorted=($(sort -g <<<"${Se_z[*]}"))
    IFS=$'\n' Cd_z_sorted=($(sort -g <<<"${Cd_z[*]}"))
    
    Zn_largest=${Cd_z_sorted[-1]}
    Zn_smallest=${Cd_z_sorted[0]}
    
    S_largest=${Se_z_sorted[-1]}
    S_smallest=${Se_z_sorted[0]}
    S4_largest=${Se_z_sorted[-1]}
    S4_smallest=${Se_z_sorted[0]}
    
    Cd_largest=${Cd_z_sorted[-2]}
    Cd_smallest=${Cd_z_sorted[1]}
...
function process_files {
    ...
    local vol0shell=$6
    local cellvol0=$7
    local vol0shellsmall=$8
    local unit_scale_stress=1.0
    local unit_scale_hartpera3togpa=4359.75
    ...
    local dvshell=$(echo "$vol0shellsmall * $scale_xy * $scale_xy * $scale_shell * (1 - $scale_presseval)" | bc -l)
...
function compute_initial_variables {
    ...
    vol0shellsmall=$(echo "($ZnL - $ZnS) * $cellA * $cellB *$cellC - $vol0core" | bc -l)
    echo "cellvol0=$cellvol0"
    echo "vol0core=$vol0core"
    echo "vol0shell=$vol0shell"
    echo "vol0shellsmall=$vol0shellsmall"
}
...
set_scale3=1.0  
 ...
 mpirun -np $SLURM_NTASKS cp2k.psmp -o $SCFNAME3.out -i $SCFNAME3.inp
    read result_shell result_core stress_shell stress_core scaledxyval <<< $(process_files $new_scalexy $new_scale_core $new_scale_shell $pressevalfrac $vol0core $vol0shell $cellvol0 $vol0shellsmall)
