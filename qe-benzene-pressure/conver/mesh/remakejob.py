...
filecoordlist=['../optimize/results/benzeneopt.out']
pressurelist=['180.0,']

fileendlist=['mesh2scf.sh','mesh3scf.sh','mesh4scf.sh','mesh5scf.sh','mesh6scf.sh','mesh9scf.sh']
...
#finds where to put in the alat    
    ...
    for i in range(len(filelinescalc)):
        if filelinescalc[i][0:len(pressuretrigger)]==pressuretrigger:
            changepressline=i
            break       
  #  newpressline='  press='+pressurelist[0]+'\n'
  #  filelinescalc[changepressline]=newpressline
