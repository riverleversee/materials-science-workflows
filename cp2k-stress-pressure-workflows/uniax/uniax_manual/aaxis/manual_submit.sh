...
#SBATCH --job-name=uniax-manual-a
...
RUNSP
    ...
    # run_fd_cmd.txt: srun template for run_fd_sp.sh (CP2K runs from bash, not Python subprocess)
...
# --- Main Loop (a-axis: high stress along X) ---
for (( p=$START_GPA; p<=$END_GPA; p++ )); do
    ...
    for D in "${DELTAS[@]}"; do
        ...
        echo "Processing a-axis Delta: $D GPa | Target P: $p GPa"
        ...
        if [ -z "$PREV_D" ]; then
            ...
            module load python
            module load anaconda
            python3 "$SCRIPTS_DIR/rotate_cell_and_convert.py" aaxis \
        ...
        while true; do
            ...
            python3 "$SCRIPTS_DIR/rotate_cell_preserve_frac.py" aaxis \
