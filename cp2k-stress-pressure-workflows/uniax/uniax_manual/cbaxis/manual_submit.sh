...
#SBATCH --job-name=uniax-manual-cb
#SBATCH --constraint=ib
#SBATCH --output=uniax_bc.%j.out
...
# --- Main Loop (cb-axis: high stress along Z) ---
for (( p=$START_GPA; p<=$END_GPA; p++ )); do
    ...
    for D in "${DELTAS[@]}"; do
        ...
        echo "Processing cb-axis Delta: $D GPa | Target P: $p GPa"
        ...
        if [ -z "$PREV_D" ]; then
            # --- First delta: rotate cell so (b+c) along Z; fractional coords from hydro direct conversion (no extra rotation) ---
            module purge
            module load python
            module load anaconda
            python3 "$SCRIPTS_DIR/rotate_cell_and_convert.py" cbaxis \
        ...
        # --- Target stress (cb-axis: Sxx=Syy, Szz high) ---
        SXXYY=$(bc -l <<< "($p - (1/3 * $D)) * 10000")
        SZZ=$(bc -l <<< "($p + (2/3 * $D)) * 10000")
        SXXYY_I=$(printf "%.0f" "$SXXYY")
        SZZ_I=$(printf "%.0f" "$SZZ")
        TARGET_STRESS="$SXXYY_I $SXXYY_I $SZZ_I 0 0 0"
        SED_EXTP="s/^[[:space:]]*EXTERNAL_PRESSURE.*/    EXTERNAL_PRESSURE $SXXYY_I 0 0 0 $SXXYY_I 0 0 0 $SZZ_I/"
        ...
        while true; do
            ...
            python3 "$SCRIPTS_DIR/rotate_cell_preserve_frac.py" cbaxis \
