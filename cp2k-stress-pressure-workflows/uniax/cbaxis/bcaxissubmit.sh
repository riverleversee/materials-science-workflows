...
# --- Check stall and apply isotropic scaling if fixed-atom opt stalled ---
...
for (( p=$START_GPA; p<=$END_GPA; p++ )); do
    ...
    for D in "${DELTAS[@]}"; do
        ...
        echo "Processing b+c-axis Delta: $D GPa | Target P: $p GPa"
        ...
        if [ -z "$PREV_D" ]; then
            ...
            module load python
            module load anaconda
            python3 "$(dirname "$BASE_DIR")/scripts/rotate_cell_and_convert.py" cbaxis \
        ...
        # --- Stress Tensor (Uniaxial along Z): Szz = P+2/3D, Sxx=Syy = P-1/3D ---
